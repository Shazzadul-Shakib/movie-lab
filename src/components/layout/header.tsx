...
export function Header({ onMenuToggle }: HeaderProps) {
  ...
  return (
    <header className='fixed top-0 left-0 right-0 z-50 h-16 border-b border-border bg-card shadow-sm'>
      <div className='flex h-full items-center px-2 sm:px-4 md:px-6'>
        ...
        <div className='flex items-center gap-1 sm:gap-2 ml-auto flex-shrink-0'>
          ...
          <button
            ...
            className='md:hidden flex items-center justify-center h-9 w-9 sm:h-10 sm:w-10 rounded-lg hover:bg-muted transition-colors cursor-pointer'
      ...
      {/* Mobile Search Bar */}
      {showSearch && (
        <div className='md:hidden px-3 sm:px-4 pb-3 border-t border-border bg-card'>
